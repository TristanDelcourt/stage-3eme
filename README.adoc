...
Du 12 au 20 Décembre une équipe d'informatique CDC m'a accueilli pour mon stage
...
Ce projet à commencé debut Septembre 2019 avec un objectif de mise en service mi-Mars 2020.
...
* L'UX et l'UI : Ceux qui définissent coment s'utilise l'application et à quoi elle ressemble
...
Un Sprint est donc une itération dans le developpement du logiciel.
...
Développement d'un jeu du "pendu" avec mon maître de stage : 

Voici la méthode principale du jeu developpé.

Le code source complet est disponible ici TODO
